    ...
    <header className={`bg-slate-100 h-12 flex items-center justify-end px-4 shadow fixed top-0 w-full z-10 `}>
      <div className="flex gap-3 items-center cursor-pointer">
        ...
        <div className="absolute left-2 hover:scale-105 w-10 " onClick={toggleSidebar}>
          <TiThMenu className="text-4xl w-10 border-3 rounded border-green-800 text-green-800" />
        </div>
        ...
        {showModal && (
          <div className="fixed inset-0 z-20" onClick={() => setShowModal(false)}>
            <div ref={modalRef} className="absolute top-12 lg:right-20 right-3 bg-white shadow-nav rounded-md py-2 w-40 min-w-[160px]" onClick={(e) => e.stopPropagation()}>
